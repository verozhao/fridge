...
function Keto() {
    ...
    const fetchRecipes = async () => {
      try {
        ...
        if (data.status === 'success') {
          ...
          // Filter initial recipes to only include Keto recipes
          const ketoRecipes = data.data.filter(recipe => recipe.filter && recipe.filter.toLowerCase() === 'keto');
  ...
  function handleSearch(term) {
    ...
    // Filter keto recipes based on search term (in name or ingredients)
    const filtered = recipes.filter(recipe => {
      if (recipe.filter && recipe.filter.toLowerCase() === 'keto') {
