...
function Vegan() {
    ...
    const fetchRecipes = async () => {
      try {
        ...
        if (data.status === 'success') {
          ...
          // Filter initial recipes to only include Vegan recipes
          const veganRecipes = data.data.filter(recipe => recipe.filter && recipe.filter.toLowerCase() === 'vegan');
  ...
  function handleSearch(term) {
    ...
    // Filter vegan recipes based on search term (in name or ingredients)
    const filtered = recipes.filter(recipe => {
      if (recipe.filter && recipe.filter.toLowerCase() === 'vegan') {
